  ...
  useEffect(() => {
    ...
    if (activeTab !== 'summernote' && summernoteRef.current) {
      ...
      try {
        ...
        // DOM 완전히 정리
        $editor.empty();
        $editor.removeClass('note-editor');
        $editor.removeData('summernote');
      } catch (error) {
        console.log('Summernote 제거 중 에러 (무시):', error);
        // 강제로 DOM 정리
        $editor.empty();
        $editor.removeClass('note-editor');
      }
    ...
    // 탭 전환 시 미리보기 초기화는 각 에디터의 onChange에서 처리
